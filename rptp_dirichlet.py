...
def entropy_pointwise(outputs):
    """
    R-TPT objective:
    mean_i H(p_i), i.e. pointwise entropy.
    This removes the KL-to-mean part from marginal entropy.
    """
...
def get_top_sim(sim_matrix, k=20):
    """
    sim_matrix: [1, N, N] or [N, N]
    Return top-k average similarity for each view.
    """
    if sim_matrix.dim() == 3:
...
def get_rtpt_image_features(model, images, args, cons):
    """
    Try to obtain visual features for R-TPT weighted ensemble.

    Preferred:
        model.forward_features(images) -> features
    Fallback:
        model.image_encoder(images)
    Final fallback:
        model.visual(images)

    Returned features are L2-normalized.
    """
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            if hasattr(model, "forward_features"):
                ...
                # R-TPT official code returns: clip_features, _, _
                ...
                feats = model.visual(images)

            else:
                raise AttributeError(
                    "Cannot find visual feature extractor. "
                    "Please implement model.forward_features(images), "
                    "or expose model.image_encoder / model.visual."
                )
...
def rtpt_weighted_ensemble(model, images, args, cons):
    """
    R-TPT final inference:
    1) compute logits for all augmented views
    2) compute image feature similarity matrix
    3) reliability = top-k neighbor similarity average
    4) softmax reliability / tau as weights
    5) weighted sum of logits
    """
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            outputs = model(images, cons, args)  # [N, C]

        image_features = get_rtpt_image_features(model, images, args, cons)  # [N, D]

        sim_matrix = torch.matmul(image_features, image_features.t())  # [N, N]
        score = get_top_sim(sim_matrix, k=args.rtpt_neighbor_k)  # [N]

        weight = F.softmax(score / args.rtpt_tau, dim=-1)  # [N]
        ensemble_output = torch.sum(weight.unsqueeze(-1) * outputs.float(), dim=0, keepdim=True)

    return ensemble_output


def unnormalize_clip_tensor(x):
    """
    x: normalized CLIP tensor [B, 3, H, W]
    return: image tensor in [0, 1]
    """
    device = x.device
...
def build_augmented_views_from_normalized_tensor(x_norm, data_transform, args):
    """
    Convert normalized adversarial image back to PIL,
    then re-apply AugMixAugmenter to get R-TPT views.
    Only supports batch size 1, which is already the TPT setting.
    """
    assert x_norm.size(0) == 1, "R-TPT adversarial augmentation expects batch size 1."

    x_01 = unnormalize_clip_tensor(x_norm.detach()).squeeze(0).cpu()
    pil_img = transforms.ToPILImage()(x_01)

    views = data_transform(pil_img)

    if isinstance(views, list):
        views = [v.unsqueeze(0).cuda(args.gpu, non_blocking=True) for v in views]
        views = torch.cat(views, dim=0)
    else:
        views = views.unsqueeze(0).cuda(args.gpu, non_blocking=True)
...
def test_time_tuning(model, inputs, optimizer, scaler, args, cons):
    ...
    for step_idx in range(args.tta_steps):
        ...
        # -------------------------------------------------
        # 1) original TPT or R TPT
        # -------------------------------------------------
        if 'tpt' in args.run_type or 'rtpt' in args.run_type:
            with torch.cuda.amp.autocast():
                if args.cocoop:
                ...
                if 'rtpt' in args.run_type:
                    # R-TPT: pointwise entropy only
                    loss_ent = entropy_pointwise(output)
                else:
                    # Original TPT: marginal entropy
                    loss_ent = avg_entropy(output)

                _ = conf_acc(output, args.gpu)

            loss = loss + args.lambda_tpt * loss_ent
        ...
        # -------------------------------------------------
        # 3) Dirichlet consistency + evidence control
        # -------------------------------------------------
        if args.dirichlet_consistency:
            ...
            if selected_idx is not None:
                logits_for_dir = output_clean_full[selected_idx]
            else:
...
def main_worker(gpu, args):
    ...
    for set_id in datasets:
        ...
        eval_out = test_time_adapt_eval(
            val_loader, model, model_state, optimizer, optim_state, scaler,
            args, args.disp_cons, classnames, set_id, data_transform
        )
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, cons, classnames, set_id, data_transform):
        ...
        if args.attack == 'pgd' and args.eval_mode in ['robust', 'both']:
            ...
            if 'rtpt' in args.run_type:
                ...
                if not args.cocoop:
                    ...
                    test_time_tuning(model, adv_views, optimizer, scaler, args, cons)

                    args.image = old_image

                    output_robust = rtpt_weighted_ensemble(model, adv_views, args, cons)

                else:
                    raise NotImplementedError(
                        "R-TPT robust branch for CoCoOp is not implemented in this patch."
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dirichlet_consistency', action='store_true', default=False,
                        help='use Dirichlet consistency across selected views')
    ...
    parser.add_argument('--lambda_tpt', type=float, default=1.0,
                        help='weight for original TPT marginal entropy loss')
    ...
    parser.add_argument('--rtpt_neighbor_k', type=int, default=20,
                    help='number of nearest neighbors for R-TPT reliability score')

    parser.add_argument('--rtpt_tau', type=float, default=0.01,
                        help='temperature for R-TPT reliability softmax')
    
    args = parser.parse_args()



    # R-TPT also requires augmented views, same as TPT.
    if 'tpt' in args.run_type or 'rtpt' in args.run_type:
        args.tpt = True
