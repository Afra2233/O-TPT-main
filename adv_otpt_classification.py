...
def pgd_attack(model, images, target, args, cons):
    if args.cocoop:
        raise NotImplementedError("PGD attack path currently supports non-CoCoOp models only.")
    ...
    for _ in range(args.pgd_steps):
        ...
        x_adv = clamp(x_adv, lower, upper).detach()
...
def main_worker(gpu, args):
    ...
    for set_id in datasets:
        clean_accuracy_data[set_id] = []
        clean_ece_data[set_id] = []
        robust_accuracy_data[set_id] = []
        robust_ece_data[set_id] = []
        ...
        results[set_id], result_dicts = test_time_adapt_eval(
            val_loader, model, model_state, optimizer, optim_state,
            scaler, args, args.disp_cons, classnames, set_id
        )

        clean_acc, clean_ece, _, _ = Calculator(result_dicts['clean'])
        clean_accuracy_data[set_id].append(clean_acc)
        clean_ece_data[set_id].append(clean_ece)

        if args.eval_attack and result_dicts['robust'] is not None:
            robust_acc, robust_ece, _, _ = Calculator(result_dicts['robust'])
    ...
    if args.csv_log is not None:
        ...
        custom_path = './adv_otpt_results.csv'
    ...
    if args.eval_attack:
        print("attack params: eps={} alpha={} steps={}".format(
    ...
    with open(custom_path, 'a' if file_exists else 'w', newline='') as csvfile:
        ...
        if not file_exists:
            ...
            if args.eval_attack:
                csvwriter.writerow(["attack", "pgd_eps", "pgd_alpha", "pgd_steps"])
                csvwriter.writerow(["pgd", args.pgd_eps, args.pgd_alpha, args.pgd_steps])

        csvwriter.writerow([current_datetime])
        csvwriter.writerow([""] + dataset_ids)

        clean_top1_accs = ["Clean Top-1 acc."] + ["{:.2f}".format(results[id]['clean'][0]) for id in dataset_ids]
        clean_top5_accs = ["Clean Top-5 acc."] + ["{:.2f}".format(results[id]['clean'][1]) for id in dataset_ids]
        csvwriter.writerow(clean_top1_accs)
        csvwriter.writerow(clean_top5_accs)

        final_clean_acc = ["Clean Accuracy"] + ["{:.2f}".format(clean_accuracy_data[id][0]) for id in dataset_ids]
        final_clean_ece = ["Clean ECE"] + ["{:.2f}".format(clean_ece_data[id][0]) for id in dataset_ids]
        csvwriter.writerow(final_clean_acc)
        csvwriter.writerow(final_clean_ece)
        ...
        if args.eval_attack:
            ...
            for id in dataset_ids:
                if results[id]['robust'] is not None:
                    robust_top1_accs.append("{:.2f}".format(results[id]['robust'][0]))
                    robust_top5_accs.append("{:.2f}".format(results[id]['robust'][1]))
                else:
                    robust_top1_accs.append("NA")
                    robust_top5_accs.append("NA")

                if len(robust_accuracy_data[id]) > 0:
                    robust_acc_row.append("{:.2f}".format(robust_accuracy_data[id][0]))
                    robust_ece_row.append("{:.2f}".format(robust_ece_data[id][0]))
                else:
                    robust_acc_row.append("NA")
                    robust_ece_row.append("NA")

            csvwriter.writerow(["Robust Top-1 acc."] + robust_top1_accs)
            csvwriter.writerow(["Robust Top-5 acc."] + robust_top5_accs)
            csvwriter.writerow(["Robust Accuracy"] + robust_acc_row)
            csvwriter.writerow(["Robust ECE"] + robust_ece_row)


def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, cons, classnames, set_id):
    batch_time = AverageMeter('Time', ':6.3f', Summary.NONE)

    clean_top1 = AverageMeter('CleanAcc@1', ':6.2f', Summary.AVERAGE)
    clean_top5 = AverageMeter('CleanAcc@5', ':6.2f', Summary.AVERAGE)

    if args.eval_attack:
        robust_top1 = AverageMeter('RobustAcc@1', ':6.2f', Summary.AVERAGE)
        robust_top5 = AverageMeter('RobustAcc@5', ':6.2f', Summary.AVERAGE)
        progress = ProgressMeter(
            len(val_loader),
            [batch_time, clean_top1, clean_top5, robust_top1, robust_top5],
            prefix='Test: '
        )
    else:
        progress = ProgressMeter(
    ...
    robust_result_dict = {'max_confidence': [], 'prediction': [], 'label': []} if args.eval_attack else None
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                if args.cocoop:
                    clean_output = model((image_feature, pgen_ctx), cons, args)
                else:
                    clean_output = model(image, cons, args)

        if 'ts' not in args.run_type:
            clean_softmax_output = softmax(clean_output)
        elif 'ts' in args.run_type:
            if 'ViT' in args.arch:
                clean_softmax_output = softmax(clean_output / temperature_value['ViT'])
            elif 'RN' in args.arch:
                clean_softmax_output = softmax(clean_output / temperature_value['RN'])
            else:
                ipdb.set_trace()

        # clean_confidence, clean_index = torch.max(clean_softmax_output, 1)

        # clean_result_dict['max_confidence'].append(clean_confidence.item())
        # clean_result_dict['prediction'].append(clean_index.item())
        # clean_result_dict['label'].append(target.item())
        clean_confidence, clean_index = torch.max(clean_softmax_output, 1)

        clean_result_dict['max_confidence'].extend(clean_confidence.detach().cpu().tolist())
        clean_result_dict['prediction'].extend(clean_index.detach().cpu().tolist())
        ...
        if args.eval_attack:
            adv_image = pgd_attack(model, image, target, args, cons)

            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    robust_output = model(adv_image, cons, args)

            if 'ts' not in args.run_type:
                robust_softmax_output = softmax(robust_output)
            elif 'ts' in args.run_type:
                if 'ViT' in args.arch:
                    robust_softmax_output = softmax(robust_output / temperature_value['ViT'])
                elif 'RN' in args.arch:
                    robust_softmax_output = softmax(robust_output / temperature_value['RN'])
                else:
                    ipdb.set_trace()

            # robust_confidence, robust_index = torch.max(robust_softmax_output, 1)

            # robust_result_dict['max_confidence'].append(robust_confidence.item())
            # robust_result_dict['prediction'].append(robust_index.item())
            # robust_result_dict['label'].append(target.item())
            robust_confidence, robust_index = torch.max(robust_softmax_output, 1)

            robust_result_dict['max_confidence'].extend(robust_confidence.detach().cpu().tolist())
            robust_result_dict['prediction'].extend(robust_index.detach().cpu().tolist())
            robust_result_dict['label'].extend(target.detach().cpu().tolist())
            r_acc1, r_acc5 = accuracy(robust_output, target, topk=(1, 5))
            robust_top1.update(r_acc1[0], image.size(0))
            robust_top5.update(r_acc5[0], image.size(0))
    ...
    results = {
        'clean': [clean_top1.avg, clean_top5.avg],
        'robust': [robust_top1.avg, robust_top5.avg] if args.eval_attack else None
    }

    result_dicts = {
        'clean': clean_result_dict,
        'robust': robust_result_dict
    }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Adversarial O-TPT evaluation')
    ...
    parser.add_argument('--eval_attack', action='store_true', default=False, help='evaluate PGD robustness')
    parser.add_argument('--pgd_eps', type=float, default=4 / 255, help='PGD epsilon in pixel space')
    parser.add_argument('--pgd_alpha', type=float, default=1 / 255, help='PGD step size in pixel space')
    parser.add_argument('--pgd_steps', type=int, default=10, help='number of PGD steps')
    parser.add_argument('--pgd_random_start', action='store_true', default=False, help='random start for PGD')

    parser.add_argument('--clip_impl', type=str, default='openai', choices=['openai', 'open_clip'])
