...
from clip.new_clip import load, tokenize, load_custom_checkpoint
...
class ClipImageEncoder(nn.Module):
    def __init__(self, device, arch="ViT-L/14", image_resolution=224, n_class=1000, clip_ckpt=None):
        super(ClipImageEncoder, self).__init__()
        clip, embed_dim, _ = load(arch, device=device, download_root=DOWNLOAD_ROOT)

        if clip_ckpt is not None:
            print(f"[ClipImageEncoder] Loading custom checkpoint from: {clip_ckpt}")
            clip = load_custom_checkpoint(clip, clip_ckpt)
...
class ClipTestTimeTuning(nn.Module):
    def __init__(
        ...
        if clip_ckpt is not None:
            print(f"[ClipTestTimeTuning] Loading robust/custom CLIP checkpoint from: {clip_ckpt}")
            clip = load_custom_checkpoint(clip, clip_ckpt)
    ...
    def inference(self, image, cons, args):
        use_input_grad = getattr(args, "input_grad", False)
...
def get_coop(clip_arch, test_set, device, n_ctx, ctx_init, cons, learned_cls=False, clip_ckpt=None):
